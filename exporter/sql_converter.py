...
class MySQLToPostgreSQLConverter:
    ...
    def _convert_insert(cls, stmt: str) -> str:
        ...
        # Compatibility fix: unescape \" → " inside single-quoted string values.
        #
        # Background: older versions of the exporter incorrectly escaped double
        # quotes as \" when writing SQL files.  PostgreSQL with
        # standard_conforming_strings=on treats \" inside a single-quoted literal
        # as a literal backslash followed by a double-quote, not just a
        # double-quote.  We strip the backslash here so that the correct character
        # is stored in the database.
        #
        # Note: the exporter no longer produces \" escapes, but this step is kept
        # as a safety net for any SQL files generated by older versions.
        #
        # Patterns handled (inside single-quoted strings only):
        #   \\\"  →  \"   (double-escaped: backslash was itself escaped)
        #   \"    →  "    (single-escaped: simple MySQL-style quote escape)
        result = []
        in_single_quote = False
        i = 0
        while i < len(sql):
            ...
            if char == "'":
                ...
                # Handle escaped single quote '' (two consecutive single quotes = one literal ')
                if in_single_quote and i + 1 < len(sql) and sql[i + 1] == "'":
                    # Escaped single quote inside string — output both chars and stay inside
                    result.append(char)
                ...
                next_char = sql[i + 1]
                if next_char == '\\' and i + 2 < len(sql) and sql[i + 2] == '"':
                    # \\\" → \" (the backslash was itself escaped; strip one level)
                    result.append('\\')
                    result.append('"')
                    i += 3
                    continue
                elif next_char == '"':
                    # \" → " (MySQL-style escaped double-quote; strip the backslash)
                    result.append('"')
                    i += 2
                    continue
                else:
